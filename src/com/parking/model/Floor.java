...
public class Floor extends Object3dContainer
{
	...
	public Floor(float $width, float $height, float $depth, Color4[] $sixColor4s, Boolean $useUvs, Boolean $useNormals, Boolean $useVertexColors)
	{
		...
		else
		{
			...
			_cols[4] = new Color4(197,202,217,255);
			_cols[5] = new Color4(197,202,217,255);
		}
		
		make();
	}
	
	public Floor(float $width, float $height, float $depth, Color4[] $sixColor4s)
	{
		this($width,$height,$depth, $sixColor4s, true,true,true);
	}
	
	public Floor(float $width, float $height, float $depth, Color4 color)
	{
		this($width,$height,$depth, new Color4[] { color, color, color, color, color, color }, true,true,true);
	}

	public Floor(float $width, float $height, float $depth)
	{
